...
class DeadLetterMonitorConsumerTest {
    ...
    void should_logDeadLetterDetails_when_recordReceived() {
        var record = new ConsumerRecord<>("user-details-creation.DLT", 0, 0L, "auth-user-id-123", "not-valid-json");
        record.headers()
                .add(new RecordHeader(KafkaHeaders.DLT_ORIGINAL_TOPIC, "user-details-creation".getBytes(StandardCharsets.UTF_8)))
                .add(new RecordHeader(KafkaHeaders.DLT_EXCEPTION_FQCN, "com.fasterxml.jackson.databind.exc.MismatchedInputException".getBytes(StandardCharsets.UTF_8)))
                .add(new RecordHeader(KafkaHeaders.DLT_EXCEPTION_MESSAGE, "cannot deserialize".getBytes(StandardCharsets.UTF_8)));

        consumer.onDeadLetter(record);

        assertThat(logAppender.list)
                .anySatisfy(loggingEvent -> assertThat(loggingEvent.getFormattedMessage())
                        .contains("user-details-creation.DLT")
                        .contains("auth-user-id-123")
                        .contains("user-details-creation")
    ...
    void should_notThrow_when_headersMissing() {
        var record = new ConsumerRecord<>("user-details-deletion.DLT", 0, 0L, "auth-user-id-456", "raw-payload");

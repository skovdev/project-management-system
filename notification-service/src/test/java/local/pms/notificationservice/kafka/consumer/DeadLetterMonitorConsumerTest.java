...
class DeadLetterMonitorConsumerTest {
    ...
    void should_logDeadLetterDetails_when_recordReceived() {
        var record = new ConsumerRecord<>("task-created.DLT", 0, 0L, "task-id-123", "{\"taskId\":\"task-id-123\"}");
        record.headers()
                .add(new RecordHeader(KafkaHeaders.DLT_ORIGINAL_TOPIC, "task-created".getBytes(StandardCharsets.UTF_8)))
                .add(new RecordHeader(KafkaHeaders.DLT_EXCEPTION_FQCN, "java.lang.RuntimeException".getBytes(StandardCharsets.UTF_8)))
                .add(new RecordHeader(KafkaHeaders.DLT_EXCEPTION_MESSAGE, "notification save failed".getBytes(StandardCharsets.UTF_8)));

        consumer.onDeadLetter(record);

        assertThat(logAppender.list)
                .anySatisfy(loggingEvent -> assertThat(loggingEvent.getFormattedMessage())
                        .contains("task-created.DLT")
                        .contains("task-id-123")
                        .contains("task-created")
                        .contains("java.lang.RuntimeException")
                        .contains("notification save failed"));
    }

    @Test
    @DisplayName("onDeadLetter does not throw when DLT headers are missing")
    void should_notThrow_when_headersMissing() {
        var record = new ConsumerRecord<>("project-created.DLT", 0, 0L, "project-id-456", "raw-payload");
